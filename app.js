...
let state;
try { state=JSON.parse(localStorage.getItem(STORAGE)||'{}')||{}; } catch(e) { const raw=localStorage.getItem(STORAGE); try { if(raw) localStorage.setItem(STORAGE+'_corrupt_backup',raw); } catch(_) {} state={}; }
if(!state || typeof state!=='object' || Array.isArray(state)) state={};
state.checks=(state.checks && typeof state.checks==='object' && !Array.isArray(state.checks))?state.checks:{};
state.tasks=Array.isArray(state.tasks)?state.tasks.filter(t=>t&&typeof t==='object'):[];
state.links=Array.isArray(state.links)?state.links.filter(l=>l&&typeof l==='object'):[];
...
function save(){let cur={};try{cur=JSON.parse(localStorage.getItem(STORAGE)||'{}')||{};}catch(e){cur={};}if(!cur||typeof cur!=='object'||Array.isArray(cur))cur={};cur.checks=state.checks;cur.links=state.links;cur.tasks=state.tasks;try{localStorage.setItem(STORAGE,JSON.stringify(cur));}catch(e){}updateProgress()}
function bindChecks(){
...
if ('serviceWorker' in navigator && (location.protocol === 'https:' || location.hostname === 'localhost')) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js?v=13.5.3').catch(() => {});
  });
}
